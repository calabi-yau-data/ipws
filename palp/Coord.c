...
int  IsNextDigit(){    
  ...
  if(c=='0') return -1; if((c<'0') || ('9'<c)) return 0; else return 1;
}
...
void checkDimension(int polyDim, int codim, int index){
  if (index == 1){ 
    ...
    printf("Please increase POLY_Dmax to at least %d = %d + 1\n",
	   (polyDim + 1), polyDim);
    puts("(nef.x -G requires POLY_Dmax >= dim(cone) = dim(support) + 1)");
    exit(0);}
}
...
{    int i, j, P=0, p[AMBI_Dmax]; Long W[AMBI_Dmax], G;	_B->n=_B->N-1; 
	...
	if(NW[i]) {p[P]=i; W[P++]=NW[i];}		/* non-zero weights */
     ...
     for(i=0;i<p[0];i++) _B->x[i][i]=1; while((++i)<p[1]) _B->x[i-1][i]=1;
...
{    Long W[AMBI_Dmax], *X[AMBI_Dmax], GLZ[AMBI_Dmax][AMBI_Dmax];
	...
	for(i=0;i<p[0];i++)_B->x[i][i]=1; while((++i)<_B->N)_B->x[i-1][i]=1;
	return; }
...
{    int i,j,n=0,N; for(N=0;N<P->np;N++) 
	...
	for(i=*nm;i<P->n;i++) P->x[n][i]=X[i]; n++;
