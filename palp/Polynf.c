...
{    int i,v, r=0, p=0, pri, EVsn, sn;
     ...
     EVsn=Make_Poly_Sym_NF(P,V,E,&sn,VP->p,NF,0,0,0); for(v=0;v<V->nv;v++) 
...
{    int i,j,k,r=0,det=1; GL_Long B[POLY_Dmax][POLY_Dmax],*b[POLY_Dmax],
     ...
     if(v==2){
       Long Y[POLY_Dmax];
       assert(d>1);
       for(i=0;i<d;i++)Y[i]=X[0][i]-X[1][i];
       r=NNgcd(Y[0],Y[1]); 
       for(i=2;i<d;i++) r=NNgcd(r,Y[i]); 
       assert(r==det);     }
#endif
     return det;
}
...
{    Long *Z=P->x[0],*N[4*VERT_Nmax]; int i,j,n=0; assert(P->n==4);
     for(i=0;i<V->nv;i++){
       if (V->v[i] >= V->nv) 
	 puts("Please do not use weight input with 'poly.x -C2'!");
       assert(V->v[i]<V->nv);}
 ...
 */
	...
	if (0!=vol%16) 
	  puts ("poly.x -C2 requires polytopes divisible by 2 as input!");
	assert(0==vol%16);vol/=16; /* degree */
...
{    int i,j,tot=0,reg=0,sym=0,ksum=0,sum=0,bary=0,ssroot=0,nofip=0,NR=NON_REF;
	...
	for(i=0;i<V->nv;i++){
	  if (V->v[i] >= V->nv) 
	    puts("Please do not use weight input with 'poly.x -E'!");
	  assert(V->v[i]<V->nv);}
...
void Make_Facet(PolyPointList *P,VertexNumList *V,EqList *E, int e, 
  ...
  for(v=0;v<V->nv;v++) 
    if(0==Eval_Eq_on_V(&E->e[e],P->x[V->v[v]],P->n)) {
      for(j=0;j<P->n;j++) VM[j][c]=P->x[V->v[v]][j]; 
      c++; }   
  for(j=0;j<P->n;j++) for(v=0;v<c;v++) VM[j][v]-=VM[j][c-1];
  /*  Print_Matrix(VM,P->n,c,"");*/
...
int Fano5d(PolyPointList *P,VertexNumList *V,EqList *E){  
  ...
  for(e=0;e<E->ne;e++) {int c,x=0,X[FanoProjNPmax];   /* BEGIN go over CELLS */
    ...
    if(x==d) {  /*MakeCellBasisFanoP(X);*/	/* simplical facet case */
      ...
      int i, j, k, l, goon = 1, DP[FanoProjNPmax+1]; DP[np]=1; 
      for(i=0;i<d;i++) {for(j=0;j<d;j++) M.x[j][i]=P->x[X[j]][i]; I+=1<<X[i];}
      assert(d==Make_G_for_GxMT_UT(M,G)); for(k=0;k<d;k++) {
        for(l=0;l<d;l++){Long s=0; for(i=0;i<d;i++) s+=G.x[l][i]*P->x[X[k]][i];
	  if(s!=(k==l)){if(TESTfano==1)		     /* >> test UnitBasis << */
	    {Print_LMatrix(G,"G");Print_LMatrix(M,"M");exit(0);} goon = 0;}}} 
      if (goon){
      for(k=0;k<d;k++) {DP[k]=D[X[k]]; Q->x[P->np-1][k]=0; Q->x[k][d]=0;}
	...
	if(abs(CC[c][X[n]])==1){ /*MakeCellBasisFanoP(Y)*/ /* call MakeCBF */
      ...
      for(i=0;i<d;i++) {for(j=0;j<d;j++) M.x[j][i]=P->x[Y[j]][i]; I+=1<<Y[i];}
      assert(d==Make_G_for_GxMT_UT(M,G)); for(k=0;k<d;k++) {
        for(l=0;l<d;l++){Long s=0; for(i=0;i<d;i++) s+=G.x[l][i]*P->x[Y[k]][i];
	  if(s!=(k==l)){if(TESTfano==1)		     /* >> test UnitBasis << */
	    {Print_LMatrix(G,"G");Print_LMatrix(M,"M");exit(0);} goon = 0;}}} 
      if(goon){
      for(k=0;k<d;k++) {DP[k]=D[Y[k]]; Q->x[P->np-1][k]=0; Q->x[k][d]=0;}
      Q->np=P->np; Q->x[np][d]=0; l=0; for(k=d;k<np;k++) {Q->x[k][d]=0;
        while(getNI(l,I)) l++; DP[k]=D[l];   if(TESTfano==1)printf("l=%d ",l);
	for(i=0;i<d;i++){Q->x[k][i]=0; 
	  for(j=0;j<d;j++) Q->x[k][i]+=G.x[i][j]*P->x[l][j]; } 
