...
{    int i,j,k,a,n,d,shift=1, I[W_Nmax+2], *nz=&WZ->M; 
	...
	while((i<n)&&IsDigit(c[i])) WZ->m[*nz]=10*WZ->m[*nz]+c[i++]-'0';j=0;
	...
	for(k=0;k<a;k++)Za[j]+=WZ->z[i][k]*WZ->B[k][j]; g=NNgcd(Za[j],g);}
if(Tout){printf("WZ->m[%d]=%d g=%ld  ",i,WZ->m[i],g);if(g==WZ->m[i])puts("");}
	if(g==WZ->m[i]) continue; if(g>1) for(k=0;k<a;k++) Za[j]/=g; 
...
/*	    for(i=0;i<d;i++) WZ->P->x[WZ->P->np][i]=WZ->rI[i]; / * obsolete */
	for(i=0;i<d;i++) if(WZ->rI[i]) break; if(i<d) /* rI!=0 => make P0=0 */
	...
	for(j=0;j<a;j++) A+=WZ->z[i][j]*WZ->A[j]; if(0==(A%=M)) continue; 
	...
	for(j=0;j<a;j++) {WZ->z[i][j]-=cx*WZ->w[j]; WZ->z[i][j]%=M;}
     ...
     for(j=0;j<a;j++){ X=WZ->r*WZ->A[j]; for(k=0;k<d;k++)
	X+=WZ->B[j][k]*WZ->rI[k];assert(X==1);}	      /* TEST:  r*A+B.rI==IP */
...
void Write_WH(Weight *_W, BaHo *_BH, VaHo *_VH, int rc, int tc,
  ...
  for(i=0;i<_W->M;i++){int j; fprintf(outFILE,"/Z%d: ",(int)_W->m[i]);
  ...
  if(tc&&(_P->n!=1+_VH->D)) {int D=0, d=_W->d;		/* LG non-geometric */
     ...
     if(D%d) {int g=Fgcd(D,d); fprintf(outFILE,"c/3=%d/%d\n",D/g,d/g);}
        ...
        for(i=0;i<=D;i++) {int j; fprintf(outFILE,"%sH%d:",i?" ":"",i);
...
{    int b=0,c=0,l,M[POLY_Dmax],I[POLY_Dmax],eq[W_Nmax],j,J=W->M,N=W->N,vac=0;
     for(j=0;j<J;j++) M[j]=W->m[j]; 	for(l=0;l<W->d;l++)      /* Z_d==GSO */
...
{    int i,j,k,l,c, b01=0;        /* ... from "proced" in lgotwist.c */
	...
	assert((b01==0)||(b01==2)); V->h[1][1]=(b01) ? 4 : 20; return;}
     ...
     for(i=0;i<=N;i++) mask[i]=1<<i; for(i=0;i<=WM;i++) woG[i]=woS[i]=woA[i]=0;
     for(k=0;k<ns;k++) {kgV *= W->m[k]/Fgcd(kgV,W->m[k]); mo*=W->m[k];}
     for(k=0;k<ns;k++) omega[k]=kgV/W->m[k]; R=kgV/W->d;
...
{    int i,j=0,n=0,*d; 
     for(i=1;i<X/i;i++) if(X%i==0)n++; M->n=n=2*n+(X==i*i);	/* #(Div(X)) */
...
#define	COEFF_Nmax	(d*D+2*N)		/* 22999000 for W_Nmax == 6 */
#endif
     P->A=Z->A=COEFF_Nmax; R->A=w[N-1]+1;	
	...
	for(i=0;i<n;i++) {Pint co=P->c[i]; assert(co>0); 
